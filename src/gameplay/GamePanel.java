...
public class GamePanel extends JPanel{
    ...
    public GamePanel(int width, int height, ShipLocation ol, Client client)
    {
        this.width=width;
        this.height=height;
        this.ol = ol;
    ...
    protected void paintComponent(Graphics grphcs) {
        ImageIcon icon = new ImageIcon(this.getClass().getClassLoader().getResource("assets/backgroud.png"));
    ...
    private void init()
    {
        ...
        IconHelper.setIcon(plane[0], 0);
        IconHelper.setIcon(plane[1], 1);
        add(plane[0]);
        add(plane[1]);
    }
    private void ready()
            ...
            public void run() {
                ...
                for(int i = 0 ; i <3; i++)
                {
                    try {
                        ...
                        if(t==2) client.getTransferHelper().Send("LETGO");
                    } catch (InterruptedException ex) {
                        Logger.getLogger(GamePanel.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
                remove(lb); repaint();
                addKeyListener(new GameControl(client));
            }
        });
        x.start();
    }
    ...
    public void animation(ShipLocation ol, Bullets but, Enemies e, Explodes explo)
    {
    ...
    {
        public ArrayBullets(GamePanel g)
        {
            for(int i =0 ; i< 20;i++)
            {
    ...
    {
        public ArrayEnemies(GamePanel g)
        {
            for(int i =0 ; i < 10;i++)
            {
